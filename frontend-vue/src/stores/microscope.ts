...
export const useMicroscopeStore = defineStore("microscope", () => {
  ...
  const lightStatus = ref<{ isOn: boolean; brightness: number }>({
    isOn: false,
    brightness: 100,
  });
  ...
  function updateLightStatus(isOn: boolean, brightness?: number) {
    lightStatus.value.isOn = isOn;
    if (brightness !== undefined) {
      lightStatus.value.brightness = brightness;
    }
  }
