...
export class CameraController {
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieve current camera configuration including exposure, gain, resolution, and available modes.'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiOperation({ 
    ...
    description: 'Modify camera configuration. Only provided fields will be updated. Settings are validated against camera capabilities.'
  })
  @ApiBody({
    schema: {
      example: {
  ...
  @ApiOperation({ 
    ...
    description: 'Returns URL for live camera preview stream. Stream format depends on Python service configuration (MJPEG, WebRTC, or HLS).'
  })
  @ApiResponse({ 
    status: 200, 
